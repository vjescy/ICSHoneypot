...
plc_IP_addresses = ['localhost','172.17.0.4','172.17.0.5'] # plc1 sends signals to plc2 and plc3
...
turn_list = [1,2] # array which contains the sequence to follow for sending the signals to plcs
               # ex. If the PLC has to send a signal first to PLC1 and then to PLC2, the array will be [1, 2], where
               # 1 and 2 are respectly the position of plc1 and plc2 socket in plcs!!!

first = False # If the local plc is the first to start
