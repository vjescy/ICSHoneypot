...
def initialization_variables():
    ...
    try:
        config_file = open("/home/config.json") # /home/mattia/Desktop/MAGISTRALE/TESI/TEST/PCS_case_study/physical_sim_docker/
...
def mydiff(x,t):
    ...
    v1 = 1 # vel cylinder 1 (cm/s)
    v2 = 1 # vel cylinder 2 (cm/s)
    v3 = 1 # vel cylinder 3 (cm/s)
    v4 = 1 # vel cylinder 4 (cm/s)

    command_cyl1 = actuator_states[0][0]
    command_cyl2 = actuator_states[1][0]
    command_cyl3 = actuator_states[2][0]
    command_cyl4 = actuator_states[3][0]
    ...
    # ODE system
    dxdt_1 = command_cyl1 * v1
    dxdt_2 = command_cyl2 * v2
    dxdt_3 = command_cyl3 * v3
    dxdt_4 = command_cyl4 * v4
...
if __name__ == "__main__":
    ...
    while True:
        ...
        max_extension = np.array([5,5,5,5]) # max extension of cyl1, cyl2, cyl3 and cyl4
        for i in range(len(simulated_data)):
            # check upper bound
            if simulated_data[i] > max_extension[i]:
                simulated_data[i] = max_extension[i]
            # check lower bound
